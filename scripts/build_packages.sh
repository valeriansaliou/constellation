...
pushd "$BASE_DIR" > /dev/null
    echo "Executing packages build steps for Constellation..."
    ...
    if [ $rc -eq 0 ]; then
        echo "Success: Done executing packages build steps for Constellation"
    else
        echo "Error: Failed executing packages build steps for Constellation"
    fi
popd > /dev/null
